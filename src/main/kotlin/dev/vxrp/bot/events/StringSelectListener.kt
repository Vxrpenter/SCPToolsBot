...
class StringSelectListener(val api: JDA, val config: Config, val translation: Translation) : ListenerAdapter() {
    init {
        api.listener<StringSelectInteractionEvent> { event ->
            ...
            if (launchOptionManager.checkSectionOption(LaunchOptionType.STRING_SELECT_LISTENER, LaunchOptionSectionType.TICKET_STRING_SELECT_MENUS).engage) TicketStringSelectMenus(event, config, translation)

            if (launchOptionManager.checkSectionOption(LaunchOptionType.STRING_SELECT_LISTENER, LaunchOptionSectionType.APPLICATION_STRING_SELECT_MENUS).engage) ApplicationStringSelectMenus(event, config, translation)
        }
    }
}