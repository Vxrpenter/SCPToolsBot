...
class ButtonListener(val api: JDA, val config: Config, val translation: Translation) : ListenerAdapter() {
    init {
        api.listener<ButtonInteractionEvent> { event ->
            if (event.button.id!!.startsWith("help")) {
                HelpButtons(event, config, translation).init()
            }

            if (event.button.id!!.startsWith("ticket")) {
            ...
            if (event.button.id!!.startsWith("application")) {
            ...
            if (event.button.id!!.startsWith("verify")) {
            ...
            if (event.button.id!!.startsWith("notice_of_departure")) {
            ...
            if (event.button.id!!.startsWith("regulars")) {
